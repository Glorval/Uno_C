...
void main() {
	...
	while (gameRunning == 1) {
		debugDump = fopen("dump.txt", "a");
		//Player always goes first

		fprintf(debugDump, "Start of Loop\n\n");//DEBUG RECORDING


		if (currentPlayer == 0 && AIOnly == 1) {
		...
		if (currentPlayer == 0) {//Its the players turn
			...
			if (skipped == 1) {//Did we get skipped?
				...
				fprintf(debugDump, "Player Zero was skipped.\n");//DEBUG RECORDING
				...
				while (legalPlay < 1 || legalishPlay < 1) {//So that if a player puts in an illegal play it just resets
					...
					if (PlayerInput.data[0] == 'D') {//The player wishes to draw a card
						...
						if (checkAgainstHand(PlayerInput, PlayerZero) == 1) {
							...
							printf("You played ");//Display what the user played and what they played on
				...
				if (legalPlay == 1 && legalishPlay >= 1) {//If its a legal play, do below, otherwise the player has drawn a card and we move on.
					...
					dataShifter(PlayerInput, &cardInPlay);//Put the card in play
					removeCardFromHand(PlayerInput, PlayerZero, &PlayerZero);//Remove the card from the players hand
		...
		//It has to be someones turn now, just either did the players turn or its not their turn yet, so it now HAS to be SOME ai's turn.
			...
			//AI
		fprintf(debugDump, "AI %d turn.\n", currentPlayer);//DEBUG RECORDING
		if (gameRunning == 1) {//Both in case the player wins and to get the rest of the code out one
			if (skipped == 1) {
				...
				if (AIPlay.data[0] != 'D') {//The AI can play a card
					...
					dataShifter(AIPlay, &cardInPlay);//Put the current card into play
					removeCardFromHand(AIPlay, AI[currentPlayer].Player, &AI[currentPlayer].Player);//remove the card from the hand
					AI[currentPlayer].Player[0].cardCount--;//Keep track of the card count
...
int checkAgainstHand(card playerInput, card playerHand[DECKLENGTH]){
	for (int counter = 0; counter < playerHand[0].cardCount; counter++) {
		if ((playerInput.data[0] == playerHand[counter].data[0]) && (playerInput.data[1] == playerHand[counter].data[1])) {//Does the input card match the current card in hand
			return(1);
		}
		else if (playerInput.data[0] == 'F' || playerInput.data[0] == 'W') {//Are we perhaps looking for a coloured wild instead of a normal card?
			if (playerInput.data[0] == playerHand[counter].data[0]) {
				return(1);
			}
		}
	}
...
void removeCardFromHand(card CardToBeRemoved, card Hand[DECKLENGTH], card *HandPassalong) {
	...
	if (CardToBeRemoved.data[0] == 'F' || CardToBeRemoved.data[0] == 'W') {
		int current = 0;
		while (CardToBeRemoved.data[0] != Hand[current].data[0]) {//Does this card match the wild we were playing?
			current++;
		}
...
card aiV1_2(card Hand[DECKLENGTH], card cardInPlay, int currentPlayer, int direction) {
	...
	if (legalPlay == 1) {
		...
		AIPlay.data[0] = Hand[wildPos].data[0];
	...
	else {//AI does not have a legal play so just return that we are drawing.
		AIPlay.data[0] = 'D';
		return(AIPlay);
	}
